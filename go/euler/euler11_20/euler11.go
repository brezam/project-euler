...
const numberString = `08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
...
func makeGrid(numberString string) [][]int {
	var grid [][]int
	for _, line := range strings.Split(numberString, "\n") {
...
func sliceGrid(grid [][]int, yStart, xStart, yStep, xStep, length int) []int {
...
func Euler11Solve() int {
	grid := makeGrid(numberString)
	height := len(grid)
	width := len(grid[0])
	best := math.MinInt
	prodFunc := func(a, b int) int {
		return a * b
	}
